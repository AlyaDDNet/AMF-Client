...
#include <game/client/components/tclient/amf_hud_editor.h>
...
void CVoting::Render()
{
	...
	if((!g_Config.m_ClShowVotesAfterVoting && !GameClient()->m_Scoreboard.IsActive() && TakenChoice()) || (!IsVoting() && !EditorPreview))
		return;
	if(!HudLayout::IsEnabled(HudLayout::MODULE_VOTES))
		return;
	const int Seconds = EditorPreview && !IsVoting() ? 60 : SecondsLeft();
	if(Seconds < 0)
	{
		OnReset();
		return;
	...
	// TClient
	if(g_Config.m_TcMiniVoteHud > 0 && !EditorPreview)
	{
		GameClient()->m_TClient.RenderMiniVoteHud();
		return;
	}

	const auto Layout = HudLayout::Get(HudLayout::MODULE_VOTES, 300.0f * Graphics()->ScreenAspect(), 300.0f);
	const float Scale = std::clamp(Layout.m_Scale / 100.0f, 0.25f, 3.0f);
	CUIRect View = {Layout.m_X, Layout.m_Y, 120.0f * Scale, 38.0f * Scale};
	m_HudBounds = {View.x, View.y, View.w, View.h, 3.0f * Scale};
	const int Style = HudLayout::GetStyle(HudLayout::MODULE_VOTES);
	if(Style != HudLayout::STYLE_MINIMAL)
		View.Draw(ColorRGBA(0.0f, 0.0f, 0.0f, Style == HudLayout::STYLE_CLEAN ? 0.24f : 0.4f), Style == HudLayout::STYLE_ROUNDED ? IGraphics::CORNER_ALL : IGraphics::CORNER_R, 3.0f * Scale);
	...
	ProgressProps.m_Progress = EditorPreview && !IsVoting() ? 0.25f : std::clamp((time() - m_Opentime) / (float)(m_Closetime - m_Opentime), 0.0f, 1.0f);
	...
	Ui()->DoLabel(&LeftColumn, EditorPreview && !IsVoting() ? Localize("Vote") : VoteDescription(), 6.0f * Scale, TEXTALIGN_ML, Props);

	View.HSplitTop(3.0f * Scale, nullptr, &View);
	View.HSplitTop(6.0f * Scale, &Row, &View);
	str_format(aBuf, sizeof(aBuf), "%s %s", Localize("Reason:"), EditorPreview && !IsVoting() ? "" : VoteReason());
