...
namespace
{
ColorRGBA CalculateRepresentativeColor(const CImageInfo &Info, size_t StartX, size_t StartY, size_t Width, size_t Height)
{
	const size_t PixelStep = Info.PixelSize();
	const size_t Pitch = Info.m_Width * PixelStep;
	uint64_t aColors[3] = {0, 0, 0};
	uint64_t AlphaSum = 0;
	...
	for(size_t y = StartY; y < StartY + Height; y++)
	{
		for(size_t x = StartX; x < StartX + Width; x++)
		{
			...
			const uint64_t Alpha = Info.m_pData[Offset + 3];
			if(Alpha == 0)
				continue;

			AlphaSum += Alpha;
			for(size_t c = 0; c < 3; c++)
				aColors[c] += (uint64_t)Info.m_pData[Offset + c] * Alpha;
		}
	}
	...
	return ColorRGBA(
		(float)aColors[0] / (float)AlphaSum / 255.0f,
		(float)aColors[1] / (float)AlphaSum / 255.0f,
		(float)aColors[2] / (float)AlphaSum / 255.0f,
		1.0f);
}
}
	...
	m_PlaceholderSkin("dummy")
{
	...
	m_PlaceholderSkin.m_OriginalBodyColor = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f);
	m_PlaceholderSkin.m_OriginalFeetColor = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f);
...
bool CSkins::LoadSkinData(const char *pName, CSkinLoadData &Data) const
{
	...
	Data.m_OriginalBodyColor = CalculateRepresentativeColor(Data.m_Info, 0, 0, BodyWidth, BodyHeight);
	Data.m_OriginalFeetColor = CalculateRepresentativeColor(Data.m_Info, FeetOffsetX, FeetOffsetY, FeetWidth, FeetHeight);
...
void CSkins::LoadSkinFinish(CSkinContainer *pSkinContainer, const CSkinLoadData &Data)
{
	...
	Skin.m_OriginalBodyColor = Data.m_OriginalBodyColor;
	Skin.m_OriginalFeetColor = Data.m_OriginalFeetColor;
