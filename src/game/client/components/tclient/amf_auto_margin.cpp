...
void CAmfAutoMargin::OnUpdate()
{
	...
	{
		if(g_Config.m_AmfAutoMarginManualValue <= 0)
		{
			// Keep the AMF persistence field as a whole-ms compatibility mirror,
			// while the canonical prediction margin remains in 0.1 ms units.
			g_Config.m_AmfAutoMarginManualValue = std::clamp((g_Config.m_ClPredictionMargin + 5) / 10, 1, 300);
		}
		else if(!AutoMarginEnabled)
		{
			g_Config.m_ClPredictionMargin = std::clamp(g_Config.m_AmfAutoMarginManualValue * 10, 1, MAX_MARGIN);
		}

		m_LastManualMargin = std::clamp(g_Config.m_ClPredictionMargin, 1, MAX_MARGIN);
	...
	if(!AutoMarginEnabled)
	{
		...
		{
			g_Config.m_AmfAutoMarginManualValue = std::clamp((CurrentManualMargin + 5) / 10, 1, 300);
	...
	if(!m_WasEnabled)
	{
		m_SavedMargin = std::clamp(g_Config.m_ClPredictionMargin, 1, MAX_MARGIN);
		m_WasEnabled = true;
	}
