...
namespace
{
	...
	constexpr float MOUSE_INDICATOR_SETTINGS_POPUP_HEIGHT = 228.0f;
...
CUIRect CAmfHudEditor::GetFallbackModuleRect(HudLayout::EModule Module, bool *pHasLiveRect) const
{
	...
	{
		const auto Bounds = GameClient()->m_Hud.GetAmfMouseIndicatorBounds();
		Rect = {Bounds.m_X, Bounds.m_Y, Bounds.m_W, Bounds.m_H};
		break;
	}
	...
	if(Rect.w <= 0.0f || Rect.h <= 0.0f)
	{
		...
		case HudLayout::MODULE_KEYSTROKES_KEYBOARD:
			...
			break;
		case HudLayout::MODULE_KEYSTROKES_MOUSE:
			Rect = {Layout.m_X, Layout.m_Y, 39.9f * Scale, 58.94f * Scale};
			break;
		case HudLayout::MODULE_PING:
...
CUi::EPopupMenuFunctionResult CAmfHudEditor::PopupModuleSettings(void *pContext, CUIRect View, bool Active)
{
	...
	const bool IsMouseIndicator = pThis->m_SelectedModule == HudLayout::MODULE_KEYSTROKES_MOUSE;
	...
	if(IsMouseIndicator)
	{
		CUIRect MouseStyleLabel, MouseStyleButtons;
		View.HSplitTop(6.0f, nullptr, &View);
		View.HSplitTop(12.0f, &MouseStyleLabel, &View);
		pThis->Ui()->DoLabel(&MouseStyleLabel, TCLocalize("Style", "AMF Client"), 8.0f, TEXTALIGN_ML);
		View.HSplitTop(16.0f, &MouseStyleButtons, &View);
		constexpr float MouseStyleButtonGap = 2.0f;
		const float MouseStyleButtonWidth = std::max(0.0f, (MouseStyleButtons.w - MouseStyleButtonGap * 2.0f) / 3.0f);
		const int CurrentPreset = std::clamp(g_Config.m_AmfMouseIndicatorPreset, 1, 3);
		CUIRect ArrowButton, DotDotButton, NothingButton;
		MouseStyleButtons.VSplitLeft(MouseStyleButtonWidth, &ArrowButton, &MouseStyleButtons);
		MouseStyleButtons.VSplitLeft(MouseStyleButtonGap, nullptr, &MouseStyleButtons);
		MouseStyleButtons.VSplitLeft(MouseStyleButtonWidth, &DotDotButton, &MouseStyleButtons);
		MouseStyleButtons.VSplitLeft(MouseStyleButtonGap, nullptr, &MouseStyleButtons);
		NothingButton = MouseStyleButtons;
		if(pThis->Ui()->DoButton_PopupMenu(&pThis->m_MouseIndicatorArrowButton, TCLocalize("Arrow", "AMF Client"), &ArrowButton, 7.0f, TEXTALIGN_MC, 0.0f, CurrentPreset == 1))
			g_Config.m_AmfMouseIndicatorPreset = 1;
		if(pThis->Ui()->DoButton_PopupMenu(&pThis->m_MouseIndicatorDotDotButton, TCLocalize("Dot Dot", "AMF Client"), &DotDotButton, 7.0f, TEXTALIGN_MC, 0.0f, CurrentPreset == 2))
			g_Config.m_AmfMouseIndicatorPreset = 2;
		if(pThis->Ui()->DoButton_PopupMenu(&pThis->m_MouseIndicatorNothingButton, TCLocalize("Nothing", "AMF Client"), &NothingButton, 7.0f, TEXTALIGN_MC, 0.0f, CurrentPreset == 3))
			g_Config.m_AmfMouseIndicatorPreset = 3;

		CUIRect PressedColorLabel, PressedColorButtons;
		View.HSplitTop(6.0f, nullptr, &View);
		View.HSplitTop(12.0f, &PressedColorLabel, &View);
		pThis->Ui()->DoLabel(&PressedColorLabel, TCLocalize("Pressed Color", "AMF Client"), 8.0f, TEXTALIGN_ML);
		View.HSplitTop(16.0f, &PressedColorButtons, &View);
		CUIRect YellowButton, WhiteButton;
		PressedColorButtons.VSplitMid(&YellowButton, &WhiteButton, 2.0f);
		const int PressedColor = std::clamp(g_Config.m_AmfMouseIndicatorPressedColor, 0, 1);
		if(pThis->Ui()->DoButton_PopupMenu(&pThis->m_MouseIndicatorYellowButton, TCLocalize("Yellow", "AMF Client"), &YellowButton, 7.0f, TEXTALIGN_MC, 0.0f, PressedColor == 0))
			g_Config.m_AmfMouseIndicatorPressedColor = 0;
		if(pThis->Ui()->DoButton_PopupMenu(&pThis->m_MouseIndicatorWhiteButton, TCLocalize("White", "AMF Client"), &WhiteButton, 7.0f, TEXTALIGN_MC, 0.0f, PressedColor == 1))
			g_Config.m_AmfMouseIndicatorPressedColor = 1;
	}
...
void CAmfHudEditor::OpenModuleSettings(const SModuleVisual &Visual)
{
	...
	const float PopupHeight = Visual.m_Module == HudLayout::MODULE_KEYSTROKES_KEYBOARD ? KEY_INDICATOR_SETTINGS_POPUP_HEIGHT :
		Visual.m_Module == HudLayout::MODULE_KEYSTROKES_MOUSE ? MOUSE_INDICATOR_SETTINGS_POPUP_HEIGHT : SETTINGS_POPUP_HEIGHT;
