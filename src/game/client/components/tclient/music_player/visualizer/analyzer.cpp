...
float CVisualizerAnalyzer::BandEdgeHz(float T)
{
	...
	const float A = logf(std::max(1.0f, s_aBandEdges[Seg]));
	const float B = logf(std::max(1.0f, s_aBandEdges[Seg + 1]));
...
float CVisualizerAnalyzer::BandEq(float BandT)
{
	...
	const float Frac = std::clamp(Pos - (float)Seg, 0.0f, 1.0f);
	return mix(s_aBandEq[Seg], s_aBandEq[std::min(Seg + 1, 5)], Frac);
}
...
void CVisualizerAnalyzer::Configure(const SVisualizerConfig &Config)
{
	...
	Sanitized.m_SampleRate = std::max(8000, Sanitized.m_SampleRate);
	Sanitized.m_BandCount = std::clamp(Sanitized.m_BandCount, 1, MAX_VISUALIZER_BANDS);
	Sanitized.m_LowCutHz = std::max(20, Sanitized.m_LowCutHz);
	Sanitized.m_HighCutHz = std::max(Sanitized.m_LowCutHz + 100, Sanitized.m_HighCutHz);
	...
	Sanitized.m_Sensitivity = std::max(0.05f, Sanitized.m_Sensitivity);
...
void CVisualizerAnalyzer::BuildWindows()
{
	for(int i = 0; i < m_MainFftSize; ++i)
		m_vMainWindow[i] = 0.5f * (1.0f - cosf(2.0f * PI * i / std::max(1, m_MainFftSize - 1)));
	for(int i = 0; i < m_BassFftSize; ++i)
		m_vBassWindow[i] = 0.5f * (1.0f - cosf(2.0f * PI * i / std::max(1, m_BassFftSize - 1)));
}
...
void CVisualizerAnalyzer::ComputeBandDistribution()
{
	...
	const float UpperCutOff = (float)std::min(m_Config.m_HighCutHz, m_Config.m_SampleRate / 2 - 1);
		...
		if(Band > 0)
		{
			...
			if(PrevBand < m_BassBandCount)
			{
				...
				if(!FirstBand && Band < m_BassBandCount && m_vBassLowerCutOff[Band] <= m_vBassLowerCutOff[PrevBand])
					m_vBassLowerCutOff[Band] = std::min(m_vBassLowerCutOff[PrevBand] + 1, BassNyquist);
				if(m_vBassUpperCutOff[PrevBand] < m_vBassLowerCutOff[PrevBand])
					m_vBassUpperCutOff[PrevBand] = std::min(m_vBassLowerCutOff[PrevBand] + 1, BassNyquist);
			}
			else if(PrevBand < m_Config.m_BandCount)
			{
				if(Band < m_Config.m_BandCount)
					m_vMainUpperCutOff[PrevBand] = std::min(m_vMainLowerCutOff[Band] - 1, MainNyquist);
				else
				...
				if(!FirstBand && Band < m_Config.m_BandCount && m_vMainLowerCutOff[Band] <= m_vMainLowerCutOff[PrevBand])
					m_vMainLowerCutOff[Band] = std::min(m_vMainLowerCutOff[PrevBand] + 1, MainNyquist);
				if(m_vMainUpperCutOff[PrevBand] < m_vMainLowerCutOff[PrevBand])
					m_vMainUpperCutOff[PrevBand] = std::min(m_vMainLowerCutOff[PrevBand] + 1, MainNyquist);
			}
		}

		const float Relative = Band < m_BassBandCount ?
			(float)m_vBassLowerCutOff[std::min(Band, m_Config.m_BandCount - 1)] / std::max(1.0f, (float)BassNyquist) :
			(float)m_vMainLowerCutOff[std::min(Band, m_Config.m_BandCount - 1)] / std::max(1.0f, (float)MainNyquist);
	...
	for(int Band = 0; Band < m_Config.m_BandCount; ++Band)
	{
		...
		const float Width = (float)std::max(1, Upper - Lower + 1);
		const float CenterFrequency = std::max(40.0f, (m_vCutOffFrequency[Band] + m_vCutOffFrequency[Band + 1]) * 0.5f);
		const float FreqBoost = powf(CenterFrequency, 0.76f);
		const float WidthNorm = powf(Width, 0.78f);
		const float SizeNorm = powf(std::max(1.0f, log2f((float)(IsBass ? m_BassFftSize : m_MainFftSize))), 0.88f);
		const float RelativeFrequency = std::clamp(CenterFrequency / std::max(1.0f, UpperCutOff), 0.0f, 1.0f);
		...
		m_vEq[Band] = HighTilt * LowLift * BassWindowLift * FreqBoost / std::max(1.0f, WidthNorm * SizeNorm);
	}
}
...
void CVisualizerAnalyzer::PushMonoSamples(const float *pSamples, int NumSamples)
{
	...
	for(int i = 0; i < NumSamples; ++i)
	{
		...
		m_RingCount = std::min(m_RingCount + 1, m_BassFftSize);
	}
}
...
void CVisualizerAnalyzer::CopyLatestSamples(float *pDst, int Count) const
{
	...
	const int Available = std::min(Count, m_RingCount);
...
void CVisualizerAnalyzer::Analyze(SVisualizerFrame &OutFrame)
{
	...
	for(int i = 0; i < AnalyzeWindow; ++i)
	{
		...
		Peak = std::max(Peak, absolute(Sample));
		InputEnergy += Sample * Sample;
		const float WindowMul = 0.5f - 0.5f * cosf((2.0f * PI * i) / std::max(1, AnalyzeWindow - 1));
	...
	for(int Band = 0; Band < BandCount; ++Band)
	{
		...
		const float FMin = std::max(BandEdgeHz(T0), BinHz);
		const float FMax = std::min(BandEdgeHz(T1), SampleRate * 0.49f);
		if(FMax <= FMin)
			continue;
		...
		const float Half = std::max(1.0f, (FMax - FMin) * 0.5f);
		...
		for(int K = KMin; K <= KMax; ++K)
		{
			...
			const float Weight = std::max(0.15f, 1.0f - DistNorm);
		...
		const float Clean = std::max(0.0f, (Level - Noise * 1.25f) * GlobalRmsGain);
		...
		BandPeak = std::max(BandPeak, 1e-5f);
...
void BuildRenderBars(const SVisualizerFrame &Frame, float *pOutBars, int RequestedBarCount)
{
	...
	for(int Bar = 0; Bar < RequestedBarCount; ++Bar)
	{
		...
		const int End = std::max(Start + 1, ((Bar + 1) * MAX_VISUALIZER_BANDS) / RequestedBarCount);
		float Sum = 0.0f;
		float Peak = 0.0f;
		int Count = 0;
