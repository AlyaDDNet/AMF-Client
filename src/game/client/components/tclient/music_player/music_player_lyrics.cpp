...
namespace
{
	...
	static constexpr int64_t LYRICS_SEEK_BACKWARD_RESET_MS = 100;
	...
	// LRC offset is a document-level, signed millisecond adjustment. It is
	// deliberately applied while parsing, not as a player-wide correction:
	// different lyric files can legitimately carry different offsets.
	static bool ParseLrcOffsetTag(const char *pLineStart, const char *pLineEnd, int64_t &OutOffsetMs)
	{
		static constexpr char aTag[] = "offset:";
		if(pLineStart == nullptr || pLineEnd == nullptr || pLineStart >= pLineEnd || *pLineStart != '[')
			return false;

		const char *p = pLineStart + 1;
		for(const char *pTag = aTag; *pTag != '\0'; ++pTag, ++p)
		{
			if(p >= pLineEnd || std::tolower((unsigned char)*p) != *pTag)
				return false;
		}
		...
		if(p >= pLineEnd || !std::isdigit((unsigned char)*p))
			return false;

		int64_t OffsetMs = 0;
		while(p < pLineEnd && std::isdigit((unsigned char)*p))
		{
			const int Digit = *p - '0';
			if(OffsetMs > (std::numeric_limits<int64_t>::max() - Digit) / 10)
				return false;
			OffsetMs = OffsetMs * 10 + Digit;
			++p;
		}
		while(p < pLineEnd && (*p == ' ' || *p == '\t'))
			++p;
		...
		OutOffsetMs = Sign > 0 ? OffsetMs : -OffsetMs;
		return true;
	}

	static int64_t ApplyLrcOffset(int64_t TimestampMs, int64_t OffsetMs)
	{
		if(OffsetMs >= 0)
			return TimestampMs > std::numeric_limits<int64_t>::max() - OffsetMs ? std::numeric_limits<int64_t>::max() : TimestampMs + OffsetMs;
		return TimestampMs < -OffsetMs ? 0 : TimestampMs + OffsetMs;
	}
} // namespace
...
float CMusicPlayerLyrics::PreferredTextSlotWidth(ITextRender *pTextRender, float FontSize, float MaxWidth, float Scale, float WidthScale) const
{
	...
	const char *pText = ShowNoMedia ? TCLocalize("No media", "AMF Client") : FallbackText(FALLBACK_TITLE);
...
bool CMusicPlayerLyrics::ParseSyncedLyrics(const char *pSyncedLyrics, std::vector<SLine> &vOut)
{
	...
	while(*p)
	{
		...
		int64_t ParsedOffsetMs = 0;
		if(ParseLrcOffsetTag(pLineStart, pLineEnd, ParsedOffsetMs))
		{
			OffsetMs = ParsedOffsetMs;
			continue;
		}
	...
	{
		for(SLine &Line : vOut)
			Line.m_StartMs = ApplyLrcOffset(Line.m_StartMs, OffsetMs);
	}
...
void CMusicPlayerLyrics::SyncMediaClock(int64_t SnapshotPositionMs, int64_t DurationMs, bool Playing, bool ForceReset)
{
	...
	const int64_t PreviousPositionMs = CurrentPositionMs();
	// The player clock only moves backwards for an accepted seek/track reset;
	// stale media samples are kept monotonic before they reach lyrics. Reset the
	// display-line guard so a backward seek cannot leave an old line pinned.
	if(!ForceReset && Playing && m_ClockPlaying && SnapshotPositionMs + LYRICS_SEEK_BACKWARD_RESET_MS < PreviousPositionMs)
		ClearLayoutState();
...
void CMusicPlayerLyrics::Render(ITextRender *pTextRender, CUi *pUi, const CUIRect &Area, float FontSize, float Delta, float CountdownCenterX)
{
	...
	case EDisplayState::Idle:
		pStatusText = TCLocalize("No media", "AMF Client");
	...
	const float CountdownTextCenterX = std::isfinite(CountdownCenterX) ? CountdownCenterX : CenterX;
	float TextStartX;
	if(TitleMarquee)
	{
		TextStartX = Area.x - m_TitleMarqueeOffset;
	}
	else if(IsCountdownIndex(m_CurrentLineIndex))
	{
		TextStartX = CountdownTextCenterX - m_BaseLineWidth * 0.5f;
	}
	...
	const float SlideT = SmoothUiEaseOutCubic(m_LineTransitionT);
	...
	if(m_OutgoingLineIndex != LINE_NONE && m_LineTransitionT < 1.0f)
	{
		...
		const float OutX = IsCountdownIndex(m_OutgoingLineIndex) ? CountdownTextCenterX - OutWidth * 0.5f : CenterX - OutWidth * 0.5f;
