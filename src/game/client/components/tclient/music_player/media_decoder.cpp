...
namespace
{
	...
	int64_t FfmpegSeek(void *pOpaque, int64_t Offset, int Whence)
	{
		...
		if(BaseWhence == SEEK_SET)
			NewPos = (size_t)std::max<int64_t>(0, Offset);
		else if(BaseWhence == SEEK_CUR)
			NewPos = (size_t)std::max<int64_t>(0, (int64_t)NewPos + Offset);
		else if(BaseWhence == SEEK_END)
			NewPos = (size_t)std::max<int64_t>(0, (int64_t)pReader->m_Size + Offset);
		else
			return AVERROR(EINVAL);
	...
	void ClampImageSize(int Width, int Height, int MaxDimension, int &ScaledWidth, int &ScaledHeight)
	{
		...
		const double Scale = std::min((double)MaxDimension / (double)Width, (double)MaxDimension / (double)Height);
		ScaledWidth = std::max(1, round_to_int((double)Width * Scale));
		ScaledHeight = std::max(1, round_to_int((double)Height * Scale));
	}
		...
		auto ConsumeFrame = [&]() {
			...
			if(DecodeAllFrames)
			{
				...
				if(LastValidPts != AV_NOPTS_VALUE && CurPts != AV_NOPTS_VALUE)
				{
					...
					if(PtsDiffTs > 0)
					{
						...
						if(Rescaled > 0)
							PtsDiffMs = (int)std::min<int64_t>(Rescaled, MEDIA_MAX_FRAME_MS);
					}
				}
				...
				if(PacketDurationTs > 0)
				{
					...
					if(Rescaled > 0)
						PacketDurationMs = (int)std::min<int64_t>(Rescaled, MEDIA_MAX_FRAME_MS);
				}
			...
			// Adaptive fallback: if we ever see a sane faster duration, use it for subsequent missing-timing frames.
			{
				...
				if(DurationMs > 0 && DurationMs <= MaxReasonableFallbackUpdateMs)
					FallbackFrameDurationMs = std::min(FallbackFrameDurationMs, ClampFrameDurationMs(DurationMs));
			}
			...
			// Cap initial per-frame durations relative to the fallback to avoid a "slideshow" start.
			{
				...
				if(FrameIndex < StartupFrameClampCount && FallbackFrameDurationMs > 0)
				{
					...
					const int StartupMaxFrameMs = std::min(MaxStartupFrameMsAbs, ClampFrameDurationMs(FallbackFrameDurationMs * 2));
			...
			// If per-frame timing is missing/invalid, fall back to the stream's frame rate.
			{
				...
				if(Rate.num > 0 && Rate.den > 0)
				{
					...
					if(Ms > 0 && Ms <= MaxReasonableFallbackMs)
					{
						...
						FallbackFrameDurationMs = std::min(FallbackFrameDurationMs, CandidateMs);
					}
				}
			}
