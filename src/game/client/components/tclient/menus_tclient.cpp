...
void CMenus::RenderSettingsAmfClient(CUIRect MainView)
{
	...
	if(s_AmfClientPage == 0)
	{
		...
		if(s_InputDetailsSection.IsVisible())
		{
			...
			static constexpr std::array<int, 8> s_aInputModeValues = {
				AMF_INPUTS_FAST,
				AMF_INPUTS_BEST,
				AMF_INPUTS_SAIKO,
				...
				AMF_INPUTS_MEOW,
				AMF_INPUTS_CLOUD};
			std::array<const char *, 8> aInputModeNames = {
				TCLocalize("Fast"),
				TCLocalize("Best"),
				TCLocalize("Saiko"),
	...
	if(s_AmfClientPage == 1)
	{
		...
		// Visuals page keeps just the general Show CPS preference here.
		constexpr float FullKeyIndicatorSettingsHeight = MarginExtraSmall + LineSize;
