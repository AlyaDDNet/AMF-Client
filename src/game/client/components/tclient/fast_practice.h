...
public:
	...
	// Position of the real server-world Tee marker while the same client slot is
	// rendered from the local practice world.
	bool GetRegularWorldMarkerPosition(int ClientId, vec2 &Position) const;
	...
	// Local presentation state for the parallel world. These calls intentionally do
	// not send an emoticon packet or a chat command to the real server.
	bool HandleLocalEmoticon(int Emoticon);
	bool HandleLocalEyeEmote(int Emote, int DurationSeconds);
	int LocalPresentationEmote(int ClientId, bool Frozen) const;
...
private:
	...
	struct SSavedEmoticonPresentation
	{
		bool m_Active = false;
		int m_Emoticon = 0;
		int m_StartTick = -1;
	...
	// The real server must receive a stable, released fire counter while the
	// corresponding input is consumed by the practice world. Keeping one
	// baseline per connection avoids turning practice fire-counter increments
	// into real-world press edges.
	std::array<int, NUM_DUMMIES> m_aNeutralFireState{};
	std::array<bool, NUM_DUMMIES> m_aNeutralFireStateValid{};
	...
	std::array<int, MAX_CLIENTS> m_aSelectedEmote{};
	std::array<int, MAX_CLIENTS> m_aTemporaryEmote{};
	std::array<int64_t, MAX_CLIENTS> m_aTemporaryEmoteEnd{};
	std::array<SSavedEmoticonPresentation, MAX_CLIENTS> m_aSavedEmoticonPresentation{};
	...
	bool IsSpectatingOrPaused() const;
