...
{
	return g_Config.m_AmfInputMode == AMF_INPUTS_CLOUD && Amount() > 0.0f;
}

float CCloudInput::Amount() const
{
	return g_Config.m_AmfCloudInputAmount / 100.0f;
}
...
{
	return g_Config.m_AmfCloudInputOthers && Amount() > 0.0f ? 1 : 0;
}
...
void CCloudInput::ApplyOffset(const CGameClient &GameClient, int ClientId, int &Tick, float &Intra) const
{
	if(!IsActive())
		return;
	if(!GameClient.IsFastInputLocalClient(ClientId) && !g_Config.m_AmfCloudInputOthers)
		return;
