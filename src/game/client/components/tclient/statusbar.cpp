...
int CStatusBar::GetDigitsIndex(const int Value, const int Max)
{
	...
	int DigitsIndex = static_cast<int>(log10((V ? V : 1)));
...
{
	return TextRender()->TextWidth(m_FontSize, "000.0ms");
}

void CStatusBar::PingRender()
{
	char aBuf[32];
	const int Margin = Client()->PredictionMargin();
	str_format(aBuf, sizeof(aBuf), "%d.%dms", Margin / 10, std::abs(Margin % 10));
	TextRender()->Text(m_CursorX, m_CursorY, m_FontSize, aBuf);
}
