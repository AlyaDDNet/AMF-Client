...
#ifndef GAME_CLIENT_COMPONENTS_TCLIENT_AMF_HUD_EDITOR_H
#define GAME_CLIENT_COMPONENTS_TCLIENT_AMF_HUD_EDITOR_H

#include <game/client/component.h>
#include <game/client/components/hud_layout.h>
#include <game/client/ui.h>

class CAmfHudEditor : public CComponent
{
public:
...
private:
	...
	// Captured once on mouse-down. Dragging must never use a renderer's next-frame
	// bounds as a new origin, otherwise dynamic modules (notably Music Player) can
	// appear to jump beneath a stationary cursor.
	CUIRect m_DragAnchorRect{};
	...
	CButtonContainer m_aStyleButtons[HudLayout::STYLE_COUNT];
	CButtonContainer m_KeyIndicatorShowCpsButton;
	CButtonContainer m_KeyIndicatorDefaultStyleButton;
	CButtonContainer m_KeyIndicatorMinecraftStyleButton;
	CButtonContainer m_MouseIndicatorArrowButton;
	CButtonContainer m_MouseIndicatorDotDotButton;
	CButtonContainer m_MouseIndicatorNothingButton;
	CButtonContainer m_MouseIndicatorYellowButton;
	CButtonContainer m_MouseIndicatorWhiteButton;
	// A disabled renderer deliberately has no live bounds. Keep the last real
	// geometry in the editor so Enable/Disable cannot change its rect, hitbox or
	// anchor merely because the runtime draw call was skipped.
	mutable CUIRect m_aLastLiveRects[HudLayout::MODULE_COUNT]{};
	mutable int m_aLastLiveScales[HudLayout::MODULE_COUNT]{};
	mutable bool m_aHasLastLiveRect[HudLayout::MODULE_COUNT]{};

	SModuleVisual GetModuleVisual(HudLayout::EModule Module) const;
	CUIRect GetFallbackModuleRect(HudLayout::EModule Module, bool *pHasLiveRect = nullptr) const;
	...
	void SetModuleEnabled(HudLayout::EModule Module, bool Enabled);
	...
	void CacheModuleRect(HudLayout::EModule Module, const CUIRect &Rect);
	void InvalidateCachedModuleRect(HudLayout::EModule Module);
