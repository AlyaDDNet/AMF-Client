...
{
	return g_Config.m_AmfSnapTap != 0 &&
...
int CControls::ResolveSnapTapDirection(int Dummy, bool LeftPressed, bool RightPressed)
{
	if(LeftPressed == RightPressed)
	{
		...
		{
			const int64_t Delay = (time_freq() * (int64_t)g_Config.m_AmfSnapTapDelay) / 1000;
...
{
	// Saiko+ keeps a continuously refreshed local fast-input snapshot, matching
	// the donor path. Keep this separate from the existing Saiko branch below so
	// its established movement handling remains unchanged.
