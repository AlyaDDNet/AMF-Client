...
class CNamePlatePartAmfClientIndicator : public CNamePlatePartIcon
{
private:
	bool m_Right;
	bool m_NameVisible = false;
	float m_NameWidth = 0.0f;
	float m_IconHeight = 0.0f;
	float m_IconWidth = 0.0f;
	bool m_LocalPlayer = false;
	vec2 m_TargetInertiaOffset;
	mutable vec2 m_InertiaOffset;
	mutable bool m_HasBasePosition = false;
	mutable vec2 m_PreviousBasePosition;

public:
	CNamePlatePartAmfClientIndicator(CGameClient &This, bool Right) :
		CNamePlatePartIcon(This),
		m_Right(Right)
	{
		m_Padding = vec2(0.0f, DEFAULT_PADDING);
	}

	void Reset(CGameClient &This) override
	{
		(void)This;
		m_TargetInertiaOffset = vec2(0.0f, 0.0f);
		m_InertiaOffset = vec2(0.0f, 0.0f);
		m_HasBasePosition = false;
	}

	void Update(CGameClient &This, const CNamePlateData &Data) override
	{
		m_Visible = Data.m_ShowAmfClientIndicator && This.m_AmfClientIndicator.CanRenderInWorld(Data.m_ClientId) &&
			(g_Config.m_AmfClientIndicatorPosition != 0) == m_Right;
		if(!m_Visible)
		{
			m_TargetInertiaOffset = vec2(0.0f, 0.0f);
			m_InertiaOffset = vec2(0.0f, 0.0f);
		...
		m_NameVisible = Data.m_ShowName;
		m_NameWidth = This.TextRender()->TextWidth(Data.m_FontSize, Data.m_aName);
		m_IconHeight = std::clamp(Data.m_FontSize * 1.05f, 19.5f, 34.0f);
		m_IconWidth = m_IconHeight * (327.0f / 287.0f);
		m_LocalPlayer = false;
		for(const int LocalId : This.m_aLocalIds)
		{
			if(Data.m_ClientId == LocalId)
			{
				m_LocalPlayer = true;
				break;
			}
		...
		if(m_LocalPlayer)
		{
			const vec2 Velocity = This.m_aClients[Data.m_ClientId].m_Predicted.m_Vel;
			// DDNet's world Y axis points down: jumping has a negative Y velocity
			// and falling a positive one, so this sign produces the desired motion.
			m_TargetInertiaOffset = vec2(
				std::clamp(Velocity.x * 0.35f, -2.5f, 2.5f),
				std::clamp(Velocity.y * 0.25f, -2.5f, 2.5f));
		}
		else
		{
			m_TargetInertiaOffset = vec2(0.0f, 0.0f);
		...
		// The icon is an overlay and must not influence the name line when the
		// name is visible. With a hidden name it supplies a virtual name line.
		m_Size = vec2(0.0f, m_NameVisible ? 0.0f : m_IconHeight);
		m_Color = ColorRGBA(1.0f, 1.0f, 1.0f, Data.m_Color.a);
	}

	void Render(CGameClient &This, vec2 Pos) const override
	{
		const float Direction = m_Right ? 1.0f : -1.0f;
		float NameEdgeX = Pos.x;
		if(!m_NameVisible)
			NameEdgeX += Direction * m_NameWidth / 2.0f;
		const float IconCenterX = NameEdgeX + Direction * (4.0f + m_IconWidth / 2.0f);
		const vec2 TargetPosition(IconCenterX, Pos.y);
		vec2 RenderPosition = TargetPosition;
		if(m_LocalPlayer)
		{
			// BasePosition is always current. Only this small, velocity-derived local
			// offset is smoothed, so the marker cannot trail behind the nameplate.
			if(!m_HasBasePosition || distance(m_PreviousBasePosition, TargetPosition) > 64.0f)
			{
				m_InertiaOffset = vec2(0.0f, 0.0f);
				m_HasBasePosition = true;
			}
			m_PreviousBasePosition = TargetPosition;
			const float DeltaTime = std::min(This.Client()->RenderFrameTime(), 0.05f);
			const float FollowAmount = 1.0f - std::exp(-30.0f * DeltaTime);
			m_InertiaOffset = mix(m_InertiaOffset, m_TargetInertiaOffset, FollowAmount);
			RenderPosition += m_InertiaOffset;
		}
		const CUIRect IconRect = {RenderPosition.x - m_IconWidth / 2.0f, RenderPosition.y - m_IconHeight / 2.0f, m_IconWidth, m_IconHeight};
		This.m_AmfClientIndicator.RenderIcon(IconRect, m_Color.a);
	}
};
...
public:
	CNamePlatePartDirection(CGameClient &This, Direction Dir) :
		CNamePlatePartIcon(This)
	{
		// The default arrows use DDNet's original IMAGE_ARROW texture directly.
		m_Texture = This.HasCustomArrowAsset() ? This.ArrowTexture() : g_pData->m_aImages[IMAGE_ARROW].m_Id;
		m_Direction = Dir;
		switch(m_Direction)
		{
		case DIRECTION_LEFT:
...
private:
	...
	ColorRGBA m_GradientLeft = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f);
	ColorRGBA m_GradientRight = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f);
...
protected:
	bool UpdateNeeded(CGameClient &This, const CNamePlateData &Data) override
	{
		...
		const bool Gradient = AmfGradient::NicknamesEnabled();
		ColorRGBA GradientLeft, GradientRight;
		if(Gradient)
			AmfGradient::GetNicknameColors(Data.m_ClientId, &This, m_Color, GradientLeft, GradientRight);
		const bool GradientChanged = m_Gradient != Gradient ||
			(Gradient && (m_GradientLeft != GradientLeft || m_GradientRight != GradientRight));
		m_Gradient = Gradient;
		m_GradientLeft = GradientLeft;
		m_GradientRight = GradientRight;
		return m_FontSize != Data.m_FontSize || str_comp(m_aText, Data.m_aName) != 0 || GradientChanged;
	}
	void UpdateText(CGameClient &This, const CNamePlateData &Data) override
	{
		m_FontSize = Data.m_FontSize;
		...
		Cursor.m_FontSize = m_FontSize;
		if(m_Gradient)
			AmfGradient::AppendNicknameColorSplits(Cursor, m_aText, Data.m_ClientId, &This, m_Color);
		This.TextRender()->CreateOrAppendTextContainer(m_TextContainerIndex, &Cursor, m_aText);
	}
...
private:
	...
	void Init(CGameClient &This)
	{
		...
		AddPart<CNamePlatePartAmfClientIndicator>(This, false);
		AddPart<CNamePlatePartName>(This);
		AddPart<CNamePlatePartAmfClientIndicator>(This, true);
...
void CNamePlates::RenderNamePlateGame(vec2 Position, const CNetObj_PlayerInfo *pPlayerInfo, float Alpha)
{
	...
	Data.m_ShowAmfClientIndicator = GameClient()->m_AmfClientIndicator.CanRenderInWorld(pPlayerInfo->m_ClientId);
...
void CNamePlates::RenderNamePlatePreview(vec2 Position, int Dummy)
{
	...
	Data.m_ShowAmfClientIndicator = false;
...
void CNamePlates::OnRender()
{
	if(Client()->State() != IClient::STATE_ONLINE && Client()->State() != IClient::STATE_DEMOPLAYBACK)
		return;
	if(g_Config.m_AmfFocusMode && g_Config.m_AmfFocusModeHideNames)
		return;
...
#endif
	if(!g_Config.m_ClNamePlates && ShowDirection == 0 && !g_Config.m_AmfClientIndicator)
		return;
