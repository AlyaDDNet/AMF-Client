...
namespace HudLayout
{
	...
	namespace
	{
		...
		static const SModuleLayout gs_aModuleLayouts[MODULE_COUNT] = {
			...
			{0.0f, 71.190f, 100, 0, true, true, 0x66000000U, STYLE_CLASSIC}, // VOTES
			...
			// canvas-relative like most modules). These values match the current authored
			// Mouse Indicator layout and are also used by Reset All/Reset Position.
			{0.0f, 120.530f, 80, 0, true, false, 0x66000000U, STYLE_CLASSIC}, // KEYSTROKES_MOUSE
		...
		bool CanonicalEnabled(EModule Module, bool LayoutEnabled)
		{
			...
			case MODULE_KEYSTROKES_MOUSE:
				return g_Config.m_AmfKeyIndicator != 0 && g_Config.m_AmfMouseIndicator != 0;
			case MODULE_MOVEMENT_INFO:
		...
		void SetCanonicalEnabled(EModule Module, bool Enabled)
		{
			...
			case MODULE_KEYSTROKES_KEYBOARD:
				...
				break;
			case MODULE_KEYSTROKES_MOUSE:
				g_Config.m_AmfMouseIndicator = Enabled;
				break;
			case MODULE_MOVEMENT_INFO:
				if(Enabled)
				{
		...
		// right/center anchored; KEYSTROKES_MOUSE instead keeps its authored
		// HUD-pixel position - see DynamicDefaultLayout() below.
		...
		SModuleLayout DynamicDefaultLayout(EModule Module, float HudWidth, float HudHeight)
		{
			...
			case MODULE_LOCAL_TIME:
				...
				break;
			case MODULE_KEYSTROKES_MOUSE:
				// Keep the authored Mouse Indicator layout in HUD-pixel space. It is the
				// same position used by Reset All/Reset Position, not an aspect-relative
				// offset derived from the keyboard module.
				Layout.m_X = 0.0f;
				Layout.m_Y = 120.530f;
				break;
			case MODULE_KEYSTROKES_KEYBOARD:
