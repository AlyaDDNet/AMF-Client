...
/* DDNet 19.9 popup implementation kept in the original UI translation unit. */
/* (c) Magnus Auvinen. See licence.txt in the root of the distribution for more information. */
/* If you are missing that file, acquire a complete release at teeworlds.com.                */
#include "ui.h"
#include "ui_scrollregion.h"
...
void CUi::RenderPopupMenus()
{
	for(size_t i = 0; i < m_vPopupMenus.size(); ++i)
	{
		...
		if(CheckActiveItem(pId))
		{
			if(!MouseButton(0))
			{
				if(!Inside && PopupMenu.m_Props.m_CloseOnClickOutside)
				{
					ClosePopupMenu(pId);
					--i;
					continue;
		...
		const bool CloseOnEscape = PopupMenu.m_Props.m_CloseOnEscape;
		...
		if(Result != POPUP_KEEP_OPEN || (Active && CloseOnEscape && ConsumeHotkey(HOTKEY_ESCAPE)))
...
bool CUi::OnInput(const IInput::CEvent &Event)
{
	...
	if(Event.m_Flags & IInput::FLAG_PRESS)
	{
		...
		else if(Event.m_Key == KEY_BACKSPACE)
			m_HotkeysPressed |= HOTKEY_BACKSPACE;
...
bool CUi::DoScrollbarOption(const void *pId, int *pOption, const CUIRect *pRect, const char *pStr, int Min, int Max, const IScrollbarScale *pScale, unsigned Flags, const char *pSuffix)
{
	...
	// Allow precise slider adjustment with Ctrl+wheel without scrolling the containing region.
...
void CUi::RenderTime(CUIRect TimeRect, float FontSize, int Seconds, bool NotFinished, int Millis, bool TrueMilliseconds) const
{
	...
	const float SecondsWidth = std::min(TextRender()->TextWidth(FontSize, aBuf, -1, -1.0f, 0, TextSizeProps), TimeRect.w);
	...
	if(Millis >= 0 && Seconds < 60 * 60)
	{
		...
		const float MillisWidth = TextRender()->TextWidth(CentisecondFontSize, aMillis, -1, -1.0f, 0, TextSizeProps);
