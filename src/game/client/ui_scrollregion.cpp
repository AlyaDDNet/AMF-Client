...
void CScrollRegion::ScrollHereAnimated(EScrollOption Option, float DurationSeconds)
{
	ScrollHere(Option);
	if(m_RequestScrollPos >= 0.0f)
		m_RequestScrollDuration = std::clamp(DurationSeconds, 0.04f, 0.4f);
}
...
void CScrollRegion::ScrollToDirect(float ScrollPosition)
{
	m_RequestScrollPos = std::clamp(ScrollPosition, 0.0f, std::max(0.0f, MaxScroll()));
	m_RequestScrollDuration = 0.0f;
}
...
void CScrollRegion::AdvanceAnimation()
{
	if(m_RequestScrollPos >= 0.0f)
	{
		...
		m_AnimTimeMax = m_RequestScrollDuration;
		m_AnimTime = m_AnimTimeMax;
		m_RequestScrollPos = -1.0f;
		m_RequestScrollDuration = 0.0f;
	}
