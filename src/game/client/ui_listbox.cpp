...
void CListBox::DoStart(float RowHeight, int NumItems, int ItemsPerRow, int RowsPerScroll, int SelectedIndex, const CUIRect *pRect, bool Background, int BackgroundCorners, bool ForceShowScrollbar, float ColumnSpacing)
{
	CUIRect View;
	if(pRect)
		View = *pRect;
	...
	m_ColumnSpacing = std::max(0.0f, ColumnSpacing);
...
CListboxItem CListBox::DoNextRow()
{
	...
	const int RemainingItems = m_ListBoxItemsPerRow - m_ListBoxItemIndex % m_ListBoxItemsPerRow;
	const float RemainingSpacing = (RemainingItems - 1) * m_ColumnSpacing;
	m_RowView.VSplitLeft((m_RowView.w - RemainingSpacing) / RemainingItems, &Item.m_Rect, &m_RowView);
	if(RemainingItems > 1)
		m_RowView.VSplitLeft(m_ColumnSpacing, nullptr, &m_RowView);
