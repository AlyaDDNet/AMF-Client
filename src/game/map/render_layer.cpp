...
{
	if(TuneZoneColor::UsesRawBlue())
		return CRenderLayerEntityBase::GetTexture();
...
{
	const CTuneTile &TuneTile = m_pTuneTiles[y * m_pLayerTilemap->m_Width + x];
	unsigned char Index = 0;
	if(TuneZoneColor::UsesRawBlue())
		Index = TuneTile.m_Type;
	else if(TuneTile.m_Type != 0)
		Index = m_TuneColorMapper.TuneNumberToColorIndex(TuneTile.m_Number);
...
{
	Graphics()->BlendNone();
	RenderMap()->RenderTunemap(m_pTuneTiles, m_pLayerTilemap->m_Width, m_pLayerTilemap->m_Height, 32.0f, Color, (Params.m_RenderTileBorder ? TILERENDERFLAG_EXTEND : 0) | LAYERRENDERFLAG_OPAQUE, TuneZoneColor::UsesRawBlue() ? nullptr : &m_TuneColorMapper);
	Graphics()->BlendNormal();
	RenderMap()->RenderTunemap(m_pTuneTiles, m_pLayerTilemap->m_Width, m_pLayerTilemap->m_Height, 32.0f, Color, (Params.m_RenderTileBorder ? TILERENDERFLAG_EXTEND : 0) | LAYERRENDERFLAG_TRANSPARENT, TuneZoneColor::UsesRawBlue() ? nullptr : &m_TuneColorMapper);
}
