...
bool CConfigManager::Save()
{
	...
	bool Success = true;
	for(ConfigDomain ConfigDomain = ConfigDomain::START; ConfigDomain < ConfigDomain::NUM; ++ConfigDomain)
		Success = SaveDomain(ConfigDomain) && Success;
	return Success;
}

bool CConfigManager::SaveDomain(ConfigDomain ConfigDomain)
{
	if(!m_pStorage || !g_Config.m_ClSaveSettings)
		return true;
	...
	m_aConfigFile[ConfigDomain] = m_pStorage->OpenFile(IStorage::FormatTmpPath(
									   aConfigFileTmp, sizeof(aConfigFileTmp), s_aConfigDomains[ConfigDomain].m_aConfigPath),
	...
	else if(io_sync(m_aConfigFile[ConfigDomain]) != 0)
	{
	...
	else
	{
		...
		else if(!m_pStorage->RenameFile(aConfigFileTmp, s_aConfigDomains[ConfigDomain].m_aConfigPath, IStorage::TYPE_SAVE))
		{
			log_error("config", "ERROR: renaming %s to %s failed", aConfigFileTmp, s_aConfigDomains[ConfigDomain].m_aConfigPath);
			m_aFailed[ConfigDomain] = true;
		}
	}
	...
	if(m_aFailed[ConfigDomain])
	{
		if(!CloseAttempted && io_close(m_aConfigFile[ConfigDomain]) != 0)
			log_error("config", "ERROR: closing failed temporary config %s", aConfigFileTmp);
		if(!m_pStorage->RemoveFile(aConfigFileTmp, IStorage::TYPE_SAVE))
			log_error("config", "ERROR: removing failed temporary config %s", aConfigFileTmp);
	}
	m_aConfigFile[ConfigDomain] = nullptr;
	return !m_aFailed[ConfigDomain];
}

unsigned long long CConfigManager::DomainHash(ConfigDomain ConfigDomain) const
{
	unsigned long long Hash = 1469598103934665603ull;
	char aLineBuf[2048];
	for(const SConfigVariable *pVariable : m_vpAllVariables)
	{
		if(pVariable->m_ConfigDomain != ConfigDomain)
			continue;
		pVariable->Serialize(aLineBuf, sizeof(aLineBuf));
		for(const char *pChar = aLineBuf; *pChar; ++pChar)
		{
			Hash ^= (unsigned char)*pChar;
