...
public:
	/* Constants: Server Browser Sorting
		...
		SORT_NUMAMFCLIENTS - Sort by active AMF Client users.
	...
	*/
	...
	virtual void SetAmfClientCount(const NETADDR &Addr, int Count) = 0;
	virtual void ClearAmfClientCounts() = 0;
	virtual void SetAmfClientPlayerNames(const NETADDR &Addr, const std::vector<std::string> &vPlayerNames) = 0;
	virtual void ClearAmfClientPlayerNames() = 0;
	virtual bool IsAmfClientPlayer(const CServerInfo &Server, const char *pPlayerName) const = 0;
