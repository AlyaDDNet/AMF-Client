...
void CServerBrowser::SetAmfClientCount(const NETADDR &Addr, int Count)
{
	const int ClampedCount = std::clamp(Count, 0, static_cast<int>(MAX_CLIENTS));
	const auto Existing = m_AmfClientCountsByAddr.find(Addr);
	if(Existing != m_AmfClientCountsByAddr.end() && Existing->second == ClampedCount)
		return;
	m_AmfClientCountsByAddr[Addr] = ClampedCount;
	CServerEntry *pEntry = Find(Addr);
	if(pEntry != nullptr)
		pEntry->m_Info.m_AmfClientCount = ClampedCount;
	m_NeedResort = true;
}

void CServerBrowser::ClearAmfClientCounts()
{
	if(m_AmfClientCountsByAddr.empty())
		return;
	m_AmfClientCountsByAddr.clear();
	for(CServerEntry *pEntry : m_vpServerlist)
		pEntry->m_Info.m_AmfClientCount = 0;
	m_NeedResort = true;
}

void CServerBrowser::SetAmfClientPlayerNames(const NETADDR &Addr, const std::vector<std::string> &vPlayerNames)
{
	if(vPlayerNames.empty())
	{
		m_AmfClientPlayerNamesByAddr.erase(Addr);
		return;
	}
	m_AmfClientPlayerNamesByAddr.insert_or_assign(Addr, vPlayerNames);
}

void CServerBrowser::ClearAmfClientPlayerNames()
{
	m_AmfClientPlayerNamesByAddr.clear();
}

bool CServerBrowser::IsAmfClientPlayer(const CServerInfo &Server, const char *pPlayerName) const
{
	if(!pPlayerName || !pPlayerName[0])
		return false;
	for(int AddressIndex = 0; AddressIndex < Server.m_NumAddresses; ++AddressIndex)
	{
		const auto It = m_AmfClientPlayerNamesByAddr.find(Server.m_aAddresses[AddressIndex]);
		if(It == m_AmfClientPlayerNamesByAddr.end())
			continue;
		for(const std::string &PlayerName : It->second)
		{
			if(str_comp(PlayerName.c_str(), pPlayerName) == 0)
				return true;
		}
	}
	return false;
...
bool CServerBrowser::SortCompareNumAmfClients(int Index1, int Index2) const
{
	const CServerInfo &First = m_vpServerlist[Index1]->m_Info;
	const CServerInfo &Second = m_vpServerlist[Index2]->m_Info;
	if(First.m_AmfClientCount != Second.m_AmfClientCount)
		return First.m_AmfClientCount > Second.m_AmfClientCount;
	return str_comp(First.m_aAddress, Second.m_aAddress) < 0;
}
...
void CServerBrowser::Filter()
{
	...
	for(int ServerIndex = 0; ServerIndex < (int)m_vpServerlist.size(); ServerIndex++)
	{
		...
		if(g_Config.m_BrFilterEmpty && Info.m_NumFilteredPlayers == 0)
			Filtered = true;
		else if(g_Config.m_AmfBrowserFilter && Info.m_AmfClientCount == 0)
			Filtered = true;
		...
		else
		{
			...
			if(!Filtered && g_Config.m_BrFilterCountry)
			{
				...
				for(int ClientIndex = 0; ClientIndex < Info.m_NumReceivedClients; ++ClientIndex)
				{
					const auto &Client = Info.m_aClients[ClientIndex];
...
int CServerBrowser::SortHash() const
{
	...
	i |= g_Config.m_AmfBrowserFilter << 17;
	return i;
}
...
void CServerBrowser::Sort()
{
	...
	else if(g_Config.m_BrSort == IServerBrowser::SORT_NUMAMFCLIENTS)
		std::stable_sort(m_vSortedServerlist.begin(), m_vSortedServerlist.end(), CSortWrap(this, &CServerBrowser::SortCompareNumAmfClients));
...
void CServerBrowser::SetInfo(CServerEntry *pEntry, const CServerInfo &Info) const
{
	...
	pEntry->m_Info = Info;
	pEntry->m_Info.m_AmfClientCount = TmpInfo.m_AmfClientCount;
	for(int AddressIndex = 0; AddressIndex < TmpInfo.m_NumAddresses; ++AddressIndex)
	{
		const auto It = m_AmfClientCountsByAddr.find(TmpInfo.m_aAddresses[AddressIndex]);
		if(It != m_AmfClientCountsByAddr.end())
		{
			pEntry->m_Info.m_AmfClientCount = It->second;
			break;
		}
	}
	...
	std::sort(pEntry->m_Info.m_aClients, pEntry->m_Info.m_aClients + pEntry->m_Info.m_NumReceivedClients, CPlayerScoreNameLess(pEntry->m_Info.m_ClientScoreKind));
...
CServerBrowser::CServerEntry *CServerBrowser::Add(const NETADDR *pAddrs, int NumAddrs)
{
	...
	*pEntry = {};
	...
	for(int i = 0; i < NumAddrs; ++i)
	{
		const auto It = m_AmfClientCountsByAddr.find(pAddrs[i]);
		if(It != m_AmfClientCountsByAddr.end())
		{
			pEntry->m_Info.m_AmfClientCount = It->second;
			break;
...
void CServerBrowser::Update()
{
	...
	// The HTTP server list starts loading during CServerBrowser initialization.
	// If the persisted browser page is already the current type, opening the
	// page does not call RefreshBrowserTab with a type change. In that case the
	// request can complete while m_RefreshingHttp is false, leaving the browser
	// entries empty until the user presses Refresh. Materialize the completed
	// HTTP data once, without issuing another request.
	if(m_ServerlistType != TYPE_LAN && !m_HttpDataApplied && !m_pHttp->IsRefreshing() && m_pHttp->NumServers() > 0)
	{
		UpdateFromHttp();
		Sort();
		m_RefreshingHttp = false;
...
void CServerBrowser::UpdateServerFilteredPlayers(CServerInfo *pInfo) const
{
	...
	if(g_Config.m_BrFilterConnectingPlayers)
	{
		for(int ClientIndex = 0; ClientIndex < pInfo->m_NumReceivedClients; ++ClientIndex)
		{
			const auto &Client = pInfo->m_aClients[ClientIndex];
...
void CServerBrowser::UpdateServerFriends(CServerInfo *pInfo) const
{
	...
	for(int ClientIndex = 0; ClientIndex < pInfo->m_NumReceivedClients; ++ClientIndex)
	{
		auto &Client = pInfo->m_aClients[ClientIndex];
...
int CServerBrowser::DetermineOwnLocation() const
{
	...
	// Keep the last resolved automatic location as a safe fallback when a
	// manually configured value cannot be parsed. Returning an uninitialized
	// location here made estimated ping depend on stack contents.
	int OwnLocation = m_OwnLocation;
