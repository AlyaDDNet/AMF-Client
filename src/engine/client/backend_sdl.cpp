...
void CGraphicsBackend_Threaded::ProcessError(const SGfxErrorContainer &Error)
{
	...
	const std::string LogMessage = "Graphics Error:\n" + m_FatalError;
...
int CGraphicsBackend_SDL_GL::Init(const char *pName, int *pScreen, int *pWidth, int *pHeight, int *pRefreshRate, int *pFsaaSamples, int Flags, int *pDesktopWidth, int *pDesktopHeight, int *pCurrentWidth, int *pCurrentHeight, IStorage *pStorage)
{
	...
	*pFsaaSamples = 0;
	...
	const int PreviousDesktopWidth = *pDesktopWidth;
	const int PreviousDesktopHeight = *pDesktopHeight;
	const bool IsDesktopChanged = PreviousDesktopWidth == 0 || PreviousDesktopHeight == 0 || PreviousDesktopWidth != DisplayMode.w || PreviousDesktopHeight != DisplayMode.h;
	if(IsDesktopChanged)
		log_info("gfx", "Desktop resolution changed from %dx%d to %dx%d", PreviousDesktopWidth, PreviousDesktopHeight, DisplayMode.w, DisplayMode.h);

	*pDesktopWidth = DisplayMode.w;
	*pDesktopHeight = DisplayMode.h;

	SDL_Rect UsableDisplayBounds{};
	if(SDL_GetDisplayUsableBounds(*pScreen, &UsableDisplayBounds) != 0)
	{
		log_warn("gfx", "Unable to get usable bounds of screen %d, using desktop bounds instead: %s", *pScreen, SDL_GetError());
		UsableDisplayBounds.w = DisplayMode.w;
		UsableDisplayBounds.h = DisplayMode.h;
	}
	...
	// Use the desktop resolution for invalid fullscreen modes. Windowed sizes are
	// validated against the usable display bounds on every launch, not only after
	// a detected monitor change. Otherwise a config copied from another user can
	// create a window partly or entirely outside the current screen.
	if((IsFullscreen && !SupportedResolution) || *pWidth == 0 || *pHeight == 0)
	{
		if(IsFullscreen && !SupportedResolution && *pWidth > 0 && *pHeight > 0)
			log_warn("gfx", "Saved fullscreen mode %dx%d@%d is unavailable on screen %d. Using desktop mode %dx%d@%d for this launch.", *pWidth, *pHeight, *pRefreshRate, *pScreen, *pDesktopWidth, *pDesktopHeight, DisplayMode.refresh_rate);
		*pWidth = *pDesktopWidth;
		*pHeight = *pDesktopHeight;
		*pRefreshRate = DisplayMode.refresh_rate;
	}
	else if(!IsFullscreen && (*pWidth < 640 || *pHeight < 480 || *pWidth > UsableDisplayBounds.w || *pHeight > UsableDisplayBounds.h))
	{
		const int RequestedWidth = *pWidth;
		const int RequestedHeight = *pHeight;
		const int SafeBaseWidth = std::min(1280, UsableDisplayBounds.w);
		const int SafeBaseHeight = std::min(720, UsableDisplayBounds.h);
		if(*pWidth < 640 || *pHeight < 480)
		{
			*pWidth = SafeBaseWidth;
			*pHeight = SafeBaseHeight;
		}
		else
		{
			const float FitScale = std::min(UsableDisplayBounds.w / (float)*pWidth, UsableDisplayBounds.h / (float)*pHeight);
			*pWidth = std::max(1, (int)std::floor(*pWidth * FitScale));
			*pHeight = std::max(1, (int)std::floor(*pHeight * FitScale));
		}
		*pRefreshRate = DisplayMode.refresh_rate;
		log_warn("gfx", "Adjusted unsafe window size %dx%d to %dx%d for usable display bounds %dx%d", RequestedWidth, RequestedHeight, *pWidth, *pHeight, UsableDisplayBounds.w, UsableDisplayBounds.h);
	}
