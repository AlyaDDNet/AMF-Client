...
void CInput::SetSystemCursorVisible(bool Visible)
{
...
void CInput::StartTextInput()
{
	...
	// Avoid restarting an already-active text input. On Android this can rapidly
	// reopen the soft keyboard, especially with the Vulkan backend.
...
void CInput::EnsureScreenKeyboardShown()
{
	...
	// Throttle restarts so holding focus on an input does not open/close it forever.
