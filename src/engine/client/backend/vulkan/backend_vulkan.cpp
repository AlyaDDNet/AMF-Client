...
class CCommandProcessorFragment_Vulkan : public CCommandProcessorFragment_GLBase
{
	...
	[[nodiscard]] bool IsFrameBlendEnabled()
	{
		return m_pWindow != nullptr &&
			(SDL_GetWindowFlags(m_pWindow) & SDL_WINDOW_INPUT_FOCUS) != 0 &&
			!IsDemoPresentationActive() &&
			g_Config.m_AmfMotionBlurEnabled != 0 &&
			g_Config.m_AmfMotionBlur > 0;
	}
...
protected:
	...
	[[nodiscard]] bool WaitFrame()
	{
		...
		bool ResetFrameBlendHistoryRequested = FrameBlendEnabled != m_FrameBlendEnabledLastFrame;
		if(ResetFrameBlendHistoryRequested)
		{
...
public:
	...
	[[nodiscard]] bool CreateFrameBlendDescriptorSet(SFrameBlendImage &FrameBlendImage)
	{
		...
		DescriptorWrite.dstSet = FrameBlendImage.m_DescriptorSet.m_Descriptor;
	...
	[[nodiscard]] bool RenderFrameBlend(VkCommandBuffer &MainCommandBuffer)
	{
		...
		// AMF uses an intuitive 0-100% scale while keeping the maximum internal
		// blend strength bounded to avoid unstable multi-pass accumulation.
		const float TotalBlendStrength = (g_Config.m_AmfMotionBlur / 100.0f) * 4.0f;
		if(TotalBlendStrength <= 0.0f)
			return true;
		constexpr float MaxBlendAlphaPerPass = 0.85f;
		int BlendPassCount = std::max(1, (int)std::ceil(TotalBlendStrength / MaxBlendAlphaPerPass));
		float BlendAlphaPerPass = TotalBlendStrength / (float)BlendPassCount;
		...
		SDeviceMemoryBlock VKBufferMem;
		size_t BufferOff = 0;
		const size_t VertexDataSize = sizeof(CCommandBuffer::SVertex) * aVertices.size();
		if(!CreateStreamVertexBuffer(MAIN_THREAD_INDEX, VKBuffer, VKBufferMem, BufferOff, aVertices.data(), VertexDataSize))
			return false;
		if(!FlushManualVertexBufferRange(VKBufferMem, BufferOff, VertexDataSize))
			return false;
	...
	{
		if(pCommand->m_SwapBeforeCapture)
		{
			if(!*pCommand->m_pSwapped && !NextFrame())
				return false;
			*pCommand->m_pSwapped = true;
