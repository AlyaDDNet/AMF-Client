...
void CGraphics_Threaded::RenderQuadContainerEx(int ContainerIndex, int QuadOffset, int QuadDrawNum, float X, float Y, float ScaleX, float ScaleY)
{
	...
	if(IsQuadContainerBufferingEnabled())
	{
		...
		Cmd.m_State = m_State;
		Cmd.m_State.m_ScreenTL = vec2((ScreenX0 - X) / ScaleX, (ScreenY0 - Y) / ScaleY);
		Cmd.m_State.m_ScreenBR = vec2((ScreenX1 - X) / ScaleX, (ScreenY1 - Y) / ScaleY);
...
static int GetEffectiveCustomAspect(bool ApplyCustomAspect)
{
	...
	const int AspectMode = g_Config.m_AmfCustomAspectRatioMode >= 0 ?
		g_Config.m_AmfCustomAspectRatioMode :
		(g_Config.m_AmfCustomAspectRatio > 0 ? 1 : 0);
	if(AspectMode <= 0)
		return 0;
	return g_Config.m_AmfCustomAspectRatio >= 100 ? g_Config.m_AmfCustomAspectRatio : 0;
}
...
void CGraphics_Threaded::SetForcedAspect(bool Force, bool ApplyCustomAspect)
{
	...
	if(CustomAspect >= 100)
	{
		const int AspectMode = g_Config.m_AmfCustomAspectRatioMode >= 0 ?
			g_Config.m_AmfCustomAspectRatioMode :
			(g_Config.m_AmfCustomAspectRatio > 0 ? 1 : 0);
		if(AspectMode == 2 && g_Config.m_AmfCustomAspectRatioNum > 0 && g_Config.m_AmfCustomAspectRatioDen > 0)
			CustomAspectF = (float)g_Config.m_AmfCustomAspectRatioNum / (float)g_Config.m_AmfCustomAspectRatioDen;
		else
...
{
	// A custom aspect is logical (ScreenAspect override), so keep the full
	// render resolution and do not apply the legacy 4:5 forced viewport.
...
bool CGraphics_Threaded::CaptureScreen(CImageInfo &Image)
{
	Image.Free();
