...
public:
	...
	void SetAmfClientCount(const NETADDR &Addr, int Count) override;
	void ClearAmfClientCounts() override;
	void SetAmfClientPlayerNames(const NETADDR &Addr, const std::vector<std::string> &vPlayerNames) override;
	void ClearAmfClientPlayerNames() override;
	bool IsAmfClientPlayer(const CServerInfo &Server, const char *pPlayerName) const override;
...
private:
	...
	// Whether the current non-LAN HTTP data has been materialized into the
	// browser entries. This is separate from m_RefreshingHttp because the HTTP
	// client can finish its initial request before the browser tab is opened.
	bool m_HttpDataApplied = false;
	...
	std::unordered_map<NETADDR, int> m_AmfClientCountsByAddr;
	std::unordered_map<NETADDR, std::vector<std::string>> m_AmfClientPlayerNamesByAddr;
	...
	bool SortCompareNumAmfClients(int Index1, int Index2) const;
